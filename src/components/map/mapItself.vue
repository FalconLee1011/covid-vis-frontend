<template>
  <v-card elevation-10>
    ...
    <div id="map" style="position: relative; width: 100%; height: 75vh; overflow: scroll;" />
    <div class="caption disabled text--disabled">last update: {{lastUpdate}}</div>
  </v-card>
</template>
...
export default {
  ...
  methods: {
    ...
    reMapInt(raw_max, raw_min, new_max, new_min, now) { return parseInt((((now - raw_min) * (new_max - new_min)) / (raw_max - raw_min)) + new_min) },
    cleanData(d){
        for (const key in d) {
          if (key == "Total") { continue; }
          var cd = d[key];
          ...
          var r = ('0' + this.reMapInt(d["Total"].Confirmed / 3, 0, 255, 24, cd.Confirmed).toString(16)).slice(-2);
          cd.fill = "#" + r + "00" + "00";
        }
        return d
    },
