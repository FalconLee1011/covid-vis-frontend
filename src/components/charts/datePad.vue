...
export default {
  ...
  mounted() { 
    if (this.mode == 'countUp') {      
      ...
      this.timeinterval = setInterval(() => { this.epochConverter(this.data); }, 1000);
    }
    else if(this.mode == 'clock') {
      this.timeinterval = setInterval(() => { this.clock(); }, 1000);
    }
  },
  methods: {
    switchMode(){
    ...
    epochConverter(epoch) {
      ...
      let secondsInADay = 60 * 60 * 24, secondsInAHour = 60 * 60;
        
      this.days = Math.floor(timeDifference / (secondsInADay) * 1);
      this.hours = Math.floor((timeDifference % (secondsInADay)) / (secondsInAHour) * 1);
      this.mins = Math.floor(((timeDifference % (secondsInADay)) % (secondsInAHour)) / (60) * 1);
      this.secs = Math.floor((((timeDifference % (secondsInADay)) % (secondsInAHour)) % (60)));
